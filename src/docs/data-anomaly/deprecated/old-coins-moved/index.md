...
In order to identify an "outstanding" transfer, for each transfer its burn age is computed. Burn age is the amount of tokens changing address on a certain transfer, multiplied by the time since they last moved. After that, for each asset, the **top 10 transfers by age over 30 days are considered**. If the new given transfer's burn age is greater than the top-10 value, the transfer is marked as "outstanding".