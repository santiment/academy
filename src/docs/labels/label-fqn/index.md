...
- `key`: Represents the label's key. It mostly serves as a common name. For instance, centralized exchange, fund, personal wallet. A comprehensive list of available labels can be found in our academy
[here](/labels).
- `value` \[optional\]: Specifies the label's value, applicable to labels that require a value. It mostly serves as a proper name (e.g. Binance, DWF, Vitalik, etc.) or some specification of the key. Details can be found within each label's description in the academy.
