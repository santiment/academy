...
and `Writing SQL Queries` articles that can be found on the [Sanqueries Oveview](/guides/santiment-queries/introduction/) page.
...
On the [API Overview](/guides/sanapi/#overview/) page one can find information how to access the API.
...
- `sqlQueryText` - The SQL query to execute. The query must be a valid SQL query that
  can be executed against the Clickhouse database. The article [Writing SQL Queries](/guides/santiment-queries/writing-sql-queries/) is a recommended read.
