...
sidebarLabel: Rate Limits and Credits Cost
sidebarOrder: 50
---
...
The queries are executed as ordinary GraphQL API calls. The same [rate limits](/guides/sanapi/rate-limits) apply to these SQL queries as well.
...
- [Sanbase plans academy page](/guides/products-and-plans/sanbase-plans/)
...
- [API plans academy page](/guides/products-and-plans/sanapi-plans/)
...
{
  runRawSqlQuery(
    sqlQueryText: "SELECT * FROM intraday_metrics limit 5"
    sqlQueryParameters: "{}"
  ) {
    clickhouseQueryId
    rows
