...
On the [API Overview](/sanapi/#overview/) page one can find information on how to access the API.
...
- Use the [sanpy execute_sql function](https://github.com/santiment/sanpy#sql-queries-santiment-queries) to execute an SQL query and get the result as a Pandas DataFrame. The python library itself also uses the API, but provides functions that hide the details of the API.
...
> **Note**: In order to be able to run the query in the GraphiQL Explorer you need to login on [Sanbase](https://app.santiment.net?utm_source=academy) on the same browser.
