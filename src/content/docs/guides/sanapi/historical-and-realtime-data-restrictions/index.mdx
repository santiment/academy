...
- `stabilizationPeriod` - the time period after which the metric value is considered stable. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](/sanapi/fetching-metrics#stabilization-period-and-data-mutability) for details.
- `canMutate` - whether the metric value can change after the stabilization period ends. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](/sanapi/fetching-metrics#stabilization-period-and-data-mutability) for details.
