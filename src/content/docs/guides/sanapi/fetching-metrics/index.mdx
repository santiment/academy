...
- **`canMutate`** — a boolean indicating whether the metric value can change
  *after* the stabilization period ends. When `false`, a value is guaranteed to
  be immutable once the stabilization period has elapsed. When `true`,
  historical values may still be updated even after stabilization (for example,
  top-holder metrics that depend on a changing total supply).
...
| After the stabilization period, `canMutate: false` | No — the value is final |
