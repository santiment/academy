...
- [Stabilization Period and Data Mutability](#stabilization-period-and-data-mutability)
...
- `stabilizationPeriod` - the time period after which the metric value is considered stable (e.g. `"1h"`, `"24h"`). During this period, the value may be recalculated as new data (such as confirmation blocks) arrives. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](#stabilization-period-and-data-mutability) for details.
- `canMutate` - a boolean indicating whether the metric value can change after the stabilization period ends. If `true`, historical values may still be updated. If `false`, values are immutable once stabilized. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](#stabilization-period-and-data-mutability) for details.
...
- `isRestricted` - whether any time (historical or real-time) restrictions are applied.
...
## Stabilization Period and Data Mutability

Metric values may not be immediately final when first computed. Two metadata
fields describe the data stability characteristics of each metric:
...
- **`stabilizationPeriod`** — the time duration after which the metric value is
  ...
  be recalculated as new data arrives — for example, a daily metric may be
  recomputed throughout the day as new blockchain events are processed.
- **`canMutate`** — a boolean indicating whether the metric value can change
  *after* the stabilization period ends. When `false`, the value should be
  immutable once the stabilization period has elapsed. When `true`,
  historical values may still be updated even after stabilization.

Note that these fields are best-effort estimates. In rare cases — such as
discovered data issues — historical values may be recalculated regardless of
these settings.

Not all metrics have these fields specified — both fields return `null` when the
stability characteristics have not been defined for a given metric.

Together, when present, these fields define the mutability contract:

| Scenario | Value can change? |
| --- | --- |
| Within the stabilization period | Yes — regardless of `canMutate` |
| After the stabilization period, `canMutate: false` | No — the value should be immutable |
| After the stabilization period, `canMutate: true` | Yes — the value may still be updated |

You can query these fields via `getMetric` metadata:
...
Or for all metrics at once via `getAccessRestrictions`:
...
If you need guaranteed finalized data points, see the
[Finalized Metrics](/metrics/finalized) page for the `onlyFinalizedData` query
flag.
...
| fields                | Optionally give aliases to the returned fields `datetime` and `value`                                             |
...
- `fields`: If the returned field names should be something other than `datetime` or `value`, aliases can be given.
  In case of `aggregation: OHLC`, the fields that can be overridden are `valueOhlc`, `open`, `high`, `close` and `low`
  - `{datetime: "dt", value: "v"}`
...
- The existing selectors and required selectors for the metric.
...
- The [stabilization period and mutability](#stabilization-period-and-data-mutability) of the metric values.
- Whether the metric has incomplete data, is timebound, or is a label FQN metric.
...
- `minInterval` — the minimal interval between two data points (e.g. `"5m"`, `"1d"`). See [Minimal Interval](#minimal-interval).
- `defaultAggregation` — the aggregation applied when fetching data at a larger interval. See [Aggregation](#aggregation).
- `availableAggregations` — the list of supported aggregation types for this metric.
- `availableSelectors` — the selector types that can be used when querying this metric (e.g. `SLUG`, `TEXT`, `HOLDERS_COUNT`).
- `requiredSelectors` — a list of lists describing which selectors must be provided. Each
  inner list represents alternatives — you must provide one selector from each group. For
  example, `[["SLUG"], ["LABEL_FQN", "LABEL_FQNS"]]` means you need `SLUG` **and** either
  `LABEL_FQN` or `LABEL_FQNS`.
- `dataType` — either `TIMESERIES` or `HISTOGRAM`. See [Data Type](/metrics/details/data-type).
- `stabilizationPeriod` — the time after which the metric value is considered stable. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](#stabilization-period-and-data-mutability).
- `canMutate` — whether the metric value can change after the stabilization period. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](#stabilization-period-and-data-mutability).
- `hasIncompleteData` — `true` when the metric's latest data point may be incomplete. These
  are typically daily metrics computed multiple times throughout the day — for example,
  `daily_active_addresses` fetched at 18:00 only includes 3/4 of the day. The
  `includeIncompleteData` query parameter (default `false`) controls whether such data
  points are returned.
- `isTimebound` — `true` if the metric is computed on the set of coins/tokens that moved
  within a specified time period. See [Timebound](/metrics/details/timebound).
- `isLabelFqnMetric` — `true` if the metric is computed on addresses identified by a
  specific [label](/labels). These labels can identify exchanges, funds, whales, and other
  entities. Use `availableLabelFqns` to discover which labels are available for a given metric.
- `availableLabelFqns` — the list of [label FQNs](/labels/label-fqn) that can be provided in
  the `labelFqn` selector when querying this metric. Accepts an optional `slug` argument to
  filter for a specific asset.
- `availableVersions` — the available [versions](#metric-versions) for this metric.
...
```graphql explorer
{
  getMetric(metric: "active_addresses_24h") {
    metadata {
      ...
      isTimebound
      isLabelFqnMetric
      availableLabelFqns(slug: "bitcoin")
    }
  }
}
```
