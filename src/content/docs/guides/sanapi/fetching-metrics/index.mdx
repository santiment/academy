...
Some metrics have multiple versions that represent different implementations or
calculation methods. The `getMetric` query accepts an optional `version`
...
For the full guide — including version comparison with aliases, experimental
versions, and backward-compatibility guarantees — see
[Metric Versions](/sanapi/metric-versions).
