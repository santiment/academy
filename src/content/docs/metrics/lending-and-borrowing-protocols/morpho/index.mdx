...
- `morpho_supply_apy` - Supply APY, calculated as a weighted average of `morpho_vaults_apy` across all Morpho vaults for an asset, weighted by each vault's total supplied amount (`morpho_vaults_total_supplied_usd`)
...
Protocol-level supply APY metric: `morpho_supply_apy`

```graphql explorer
{
  getMetric(metric: "morpho_supply_apy") {
    timeseriesDataJson(
      slug: "weth"
      from: "2025-09-01T00:00:00Z"
