...
Let's look at the following example. Assume that Alice and Bob each have one
...
to the whole contribution, while the last consumed stack element can be only partially
consumed.
...
greater than the total outgoing amount, and outgoing otherwise. So we
