...
author: Ivan
description: Track from what price range the coins/tokens moved on a given day were acquired
...
The metric returns a list of price ranges and the number of coins/tokens that
...
of knowing exactly how many coins/tokens were last moved when the price was
...
Let's take a look at what prices were acquired at for the bitcoins transacted on 20 July 2021, when bitcoin reached a local minimum of around $29,000, after reaching over $64,000 just 3 months before that:
- `[$low - $high]` - `number of bitcoins`
- `[$0 - $15880.41]` -  `13586.16`
