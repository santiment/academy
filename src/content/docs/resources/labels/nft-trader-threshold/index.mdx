---
title: NFT trader threshold
author: Alex
description: NFT trader threshold label
datePublished: 2023-03-22
dateModified: 2025-01-24
---

## Description

The label is attached to addresses that have actively traded NFTs recently. During an ordinary market, this label duplicates the [nft_trader](/labels/nft-trader) label. It differs during the most active NFT market periods and denotes the most active portion of the traders.

While the NFT trader label is based on percentiles, the NFT trader threshold uses the top-Nth value of trades count and volume among all NFT active addresses, with the N number recorded in the value field of the label.
The labels are calculated for two N-threshold values: 1000 and 5000.


## [Label fqn](/labels/label-fqn)

`santiment/nft_trader_threshold->{your_value}:v1`

Currently only two thresholds are supported:
- `santiment/nft_trader_threshold->1000:v1`
- `santiment/nft_trader_threshold->5000:v1`


## Label Examples

The address was labeled as an NFT trader with threshold 1000 on May 3rd, 2023:
[0xb7b668c3dd82bbc8708d41146af74d415087969d](https://etherscan.io/address/0xb7b668c3dd82bbc8708d41146af74d415087969d)

## Available Blockchains

* Ethereum
