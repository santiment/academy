...
Metrics with free access have full historical and real-time data available
...
- `isRestricted` - Shows if any time (historical or real-time) restrictions are applied.
...
- `stabilizationPeriod` - The time period after which the metric value is considered stable (e.g. `"1h"`, `"24h"`). During this window, values may be recalculated as new data arrives. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](/sanapi/fetching-metrics#stabilization-period-and-data-mutability) for details.
- `canMutate` - A boolean indicating whether the metric value can change after the stabilization period ends. If `false`, the value is immutable once stabilized. Returns `null` when not specified for the metric. See [Stabilization Period and Data Mutability](/sanapi/fetching-metrics#stabilization-period-and-data-mutability) for details.
- `isDeprecated` - Whether the metric is deprecated. If `true`, you should stop using it as it will be removed in the future.
- `hardDeprecateAfter` - The datetime after which a deprecated metric will become inaccessible. May be `null` if not yet scheduled.
