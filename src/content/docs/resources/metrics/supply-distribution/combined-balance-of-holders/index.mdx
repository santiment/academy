...
> past year. To include only active holders, add the `active_` prefix (for example: `active_holders_distribution_combined_balance_<interval>`)
...
- `holders_distribution_combined_balance_<interval>` and `holders_distribution_combined_balance_over_<threshold>` - Amount in tokens
