...
- `holders_labeled_distribution_combined_balance_<interval>` - Total tokens held by all addresses with a specific [label](/resources/metrics/details/supply_distribution_parameters#available-labels)
  within a specified balance [interval](/resources/metrics/details/supply_distribution_parameters#available-intervals)
- `holders_labeled_negative_distribution_combined_balance_<interval>` - Total tokens held by all addresses that do not contain a specific
  [negative label](/resources/metrics/details/supply_distribution_parameters#available-negative-labels), within a specified balance [interval](/resources/metrics/details/supply_distribution_parameters#available-intervals)
...
```graphql explorer
{
  getMetric(metric: "holders_labeled_distribution_combined_balance_1_to_10") {
    timeseriesDataJson(
      selector: { slug: "ethereum", label: "miner" }
...
{
  getMetric(
    metric: "holders_labeled_negative_distribution_combined_balance_10_to_100"
  ) {
    timeseriesDataJson(
      selector: { slug: "ethereum", label: "nonExchange" }
      from: "2024-01-01T00:00:00Z"
      to: "2024-01-10T00:00:00Z"
      interval: "1d"
    )
