...
**Data reliability notice (August 2026):** Our GitHub-based metrics (Development Activity, GitHub Activity, and their contributors count variants) are currently broken due to an issue with our GitHub data collection, and are expected to remain unreliable for a while. The affected metrics are labeled accordingly and temporarily hidden on the Santiment platform. We will update this page once the data is restored.
...
did not beat hodling BTC overall.
