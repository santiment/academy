...
**Data reliability notice (August 2026):** The GitHub Activity Contributors Count metrics are currently broken due to an issue with our GitHub data collection, and are expected to remain unreliable for a while. The affected metrics are labeled accordingly and temporarily hidden on the Santiment platform. We will update this page once the data is restored.
